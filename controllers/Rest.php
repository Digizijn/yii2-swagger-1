...
//			$actions['view']['prepareDataProvider'] = [$this, 'indexDataProvider'];
		}
...
//		];
	...
	public function runAction($id, $params = [])
	{
		...
		if (YII_ENV_PROD || isset($_GET['log'])) {
			$request = Yii::$app->getRequest();
			Yii::info(Json::encode([
				'url' 		=> $request->getUrl(),
				'user'		=> EO::$app->getUser()->getId(),
				'headers' 	=> $request->getHeaders(),
				'params' 	=> $params,
				'response' 	=> substr(Json::encode($response), 0, 10000)
			]), 'response');
		}
	...
	public function indexDataProvider() {
		return static::filterIndexDataProvider(call_user_func([$this->modelClass, 'find']));
	}


	/** @return ActiveDataProvider */
	static public function filterIndexDataProvider($query) {
